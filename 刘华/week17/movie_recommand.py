...
def user_cf(user_id, item_id, user_to_similar_user, user_to_rating, topn=10):
    print("user_to_similar_user:", user_to_similar_user[user_id][:topn])
    # print("user_to_similar_user:", type(user_to_similar_user))
    pred_score = 0
    count = 0
    for similar_user, similarity in user_to_similar_user[user_id][:topn]:
        ...
        if (rating_by_similiar_user > 0):
            print(rating_by_similiar_user, 'rating_by_similiar_user')
...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=20):
    print("similar_items:", similar_items[user_id])
    pred_score = 0
    count = 0
    for item, similarity in similar_items[item_id][:topn]:
        # 用户对相似电影的打分
        rating_by_similiar_user = user_to_rating[user_id][item - 1]
        #分数*用户相似度，作为一种对分数的加权，越相似的用户评分越重要
        pred_score += rating_by_similiar_user * similarity
        #如果这个相似用户没看过，就不计算在总数内
        if rating_by_similiar_user != 0:
