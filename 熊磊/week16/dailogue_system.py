...
class DailogueSystem:
    ...
    def intent_recognition(self, memory):
        ...
        score_repeat = self.get_reapet_score(memory["query"]) #计算意图是否为“没听清”或“请重复一遍”
        max_score = score_repeat
        print(f"repeat score: {score_repeat}")
        for node_name in memory["available_nodes"]: # 遍历所有可能的节点，并计算它们的打分
            node_info = self.nodes_info[node_name]
            score = self.get_node_score(memory["query"], node_info) # 当前节点与query的匹配程度
            print(f"{node_name}: {score}")
            if score > max_score:
                max_score = score
                memory["hit_node"] = node_name
        if score_repeat > max_score:
            memory["policy"] = "repeat"
                
        return memory

    def get_reapet_score(self, query):
        #计算意图是否为“没听清”或“请重复一遍”
        max_score = 0
        for intent in ["没听清", "请重复一遍"]:
    ...
    def get_node_score(self, query, node_info):
        ...
        for intent in intent_list:
            score = max(score, self.sentence_match_score(query, intent)) # 计算当前节点的每个intent与query的匹配程度，取最大的
        return score
    ...
    def dpo(self, memory):
        if memory.get("policy", False) and memory["policy"] == "repeat":
            # memory["policy"] = "ask"
            # memory["available_nodes"] = [memory["hit_node"]] # 停留在当前节点，再问一次
            return memory
