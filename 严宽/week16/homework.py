...
class DialogSystem:
    def __init__(self):
        ...
        self.llm = ChatDeepSeek(model="deepseek-chat", api_key=self.api_key)

    def load(self):
        self.all_node_info = {}  # key = 节点id， value = node info
        self.load_scenrio("./scenario/scenario-买衣服.json")
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "ask":
            ...
            ## 通过Langchain 的链式调用生成一个chain
            chain = prompt | self.llm | output_parser

            result = chain.invoke({"input": "作为客服请说欢迎语"})
