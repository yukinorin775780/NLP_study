...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    sum_similarity_rating = 0.0
    sum_similarity = 0.0
    # 获取目标物品的相似物品列表（取前topn个）
    for similar_item, similarity in similar_items.get(item_id, [])[:topn]:
        # # 获取用户对相似物品的评分（假设评分存储结构为列表，索引=item_id-1）
        rating = user_to_rating[user_id][similar_item - 1]
        if rating != 0:  # 只处理用户有评分的物品
            sum_similarity_rating += rating * similarity
            sum_similarity += similarity
    # 处理分母为零的情况（若无有效相似物品则返回0）
    if sum_similarity == 0:
        return 0.0
    return sum_similarity_rating / sum_similarity
