...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    # 获取目标物品的最相似的前topn个物品及其相似度
    similar_list = similar_items.get(item_id, [])
    similar_top = similar_list[:topn]
    numerator = 0.0  # 分子：相似度与评分的乘积之和
    denominator = 0.0  # 分母：相似度绝对值之和

    for similar_item, similarity in similar_top:
        # 获取用户对相似物品的评分（注意item_id从1开始，列表索引从0开始）
        user_rating = user_to_rating[user_id][similar_item - 1]
        if user_rating != 0:  # 只考虑用户实际评分过的物品
