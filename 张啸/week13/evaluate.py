...
class Evaluator:
    ...
    def eval(self, epoch):
        self.logger.info("开始测试第%d轮模型" % epoch)
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            self.write_stats(labels, pred_result, sentences)
        self.show_stats()
        return
    
    def write_stats(self, labels, pred_result, sentences):
        assert len(labels) == len(pred_result) == len(sentences)
        if not self.config["use_crf"]:
            pred_result = torch.argmax(pred_result, dim=-1)
        for true_label, pred_label, sentence in zip(labels, pred_result, sentences):
            if not self.config["use_crf"]:
