...
def find_similar_item(user_to_rating):
    ...
    #复用一下下方的相似度计算方法
    return find_similar_user(item_to_vector,'item')

#依照user对item的打分判断user之间的相似度
def find_similar_user(user_to_rating, user_or_item='user'):
    ...
    for user_a, ratings_a in user_to_rating.items():
        ...
        for user_b, ratings_b in user_to_rating.items():
            ...
            if user_or_item == 'user':
                ...
                if user_b == user_a or user_b > 300 or user_a > 300 :
                    continue
            #ab用户互换不用重新计算cos
...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    count = 0
    # 获取评分不为0的电影
    liked_items = [i + 1 for i, rating in enumerate(user_to_rating[user_id]) if rating != 0]
    # 循环用户喜欢的电影

    for item_h in liked_items:
        # 获取用户对当前电影的评分
        user_rating = user_to_rating[user_id][item_h-1]
        
        # 查找当前电影与目标电影的相似度
        for similar_item, similarity in similar_items.get(item_h, []):
...
if __name__ == "__main__":
    user_item_score_data_path = "D:/python_project_git/ai_study/week17 推荐系统/week17 推荐系统/ml-100k/u.data"
    item_name_data_path = "D:/python_project_git/ai_study/week17 推荐系统/week17 推荐系统/ml-100k/u.item"
